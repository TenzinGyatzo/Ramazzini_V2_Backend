...
# Plan 03-01 — Backend: listar batches GIIS del proveedor

**Objetivo:** Exponer un endpoint que devuelva la lista de batches de exportación GIIS del proveedor (proveedor de salud del usuario autenticado), ordenados por mes reportado descendente, para que la UI pueda mostrar el historial. Respetar SIRES: el controller ya valida régimen en otros endpoints; el listado solo debe incluir batches del proveedor del usuario.
...
- [ ] **GET /giis-export/batches** (sin :batchId): lista de batches del proveedor. Obtener proveedor del usuario con getProveedorSaludIdFromRequest; si el proveedor no es SIRES_NOM024, devolver 403 igual que en createBatch (o devolver array vacío según política — decidir: 403 es consistente con “no mostrar la opción”).
...
- [ ] No paginación en esta fase (CONTEXT: “todos los batches del proveedor”; si en el futuro hay muchos, se puede añadir límite).
...
- No requiere guard de roles en backend si la app ya restringe por rol en frontend; el backend solo restringe por proveedor y SIRES.
</task>
...
- GET /giis-export/batches con usuario de proveedor SIRES devuelve 200 y array de batches ordenados por yearMonth desc.
- GET /giis-export/batches con usuario de proveedor no SIRES devuelve 403.
