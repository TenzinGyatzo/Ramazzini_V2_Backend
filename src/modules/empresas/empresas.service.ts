  ...
  async remove(id: string): Promise<boolean> {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        const centrosTrabajo = await this.centroTrabajoModel.find({ idEmpresa: id }).session(session).exec();
        console.log(`[DEBUG] Eliminando ${centrosTrabajo.length} Centros de Trabajo...`);
  
        if (centrosTrabajo.length > 0) {
          const resultadosEliminacion = await Promise.all(
            centrosTrabajo.map(async (centro) => this.centrosTrabajoService.remove(centro._id.toString()))
          );
        ...
        const result = await this.empresaModel.findByIdAndDelete(id).session(session);
      ...
      console.error(`[ERROR] Error al eliminar la Empresa y sus dependencias: ${error.message}`);
      session.endSession();
      return false;
    }
  }
